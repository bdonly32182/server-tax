...
const ConfirmMember = async(req,res)=>{
    let {Pers_no,password,Fname,Lname,role_name,picture,distict_member_id} = req.body
    let checkUser = await db.Employee.findOne({where:{Pers_no:Pers_no}})
    //check ว่ามียูสเซอร์ในระบบรึยัง
    if(checkUser)  res.status(404).send();
    else{
       bcrypt.genSalt(10,(err,salt)=>{
        ...
        bcrypt.hash(password,salt,async(err,hash)=>{
            ...
            res.status(200).send({msg:"ยืนยันการสมัครเรียบร้อยแล้ว"})
        })
    }) 
    }
...
const Login = async(req,res) =>{
    ...
    const user = await db.Employee.findOne({where:{Pers_no:Pers_no},include:[db.Employee_Login]})
    ...
    bcrypt.compare(password,user.Employee_Login.password,(err,isMatch)=>{
        if(err) throw err
            ...
            jwt.sign({Pers_no:user.Pers_no,distict_id:user.distict_id,role:user.role,Fname:user.Fname,Lname:user.Lname},config["jwtSecret"],{expiresIn:'1days'},
            (err,token)=>{
                if(err) throw err
                res.status(200).send({
                    token,
